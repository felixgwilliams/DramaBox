...
def parse_args():
    ...
    p.add_argument("--audio-dir", default=None, help="Base directory for audio files (if paths in index are relative)")
...
def main():
    ...
    # Parse dataset
    logger.info(f"Parsing {args.dataset_type} dataset from {args.index}...")
    ...
    logger.info(f"After duration filter [{args.min_duration}s, {args.max_duration}s]: {len(samples)} (dropped {before - len(samples)})")
    ...
    if args.num_shards > 1:
        ...
        logger.info(f"Shard {args.shard}/{args.num_shards}: {len(samples)} samples (of {total} total)")
    ...
    logger.info("Loading text encoder (Gemma + feature extractor)...")
    ...
    logger.info("Loading feature extractor (on CPU first to save GPU memory)...")
    ...
    logger.info("Encoding text prompts (Blocks 1+2: Gemma + feature extractor)...")
    for i, sample in enumerate(samples):
        ...
        video_feats, audio_feats = text_encoder.feature_extractor(hidden_states, attention_mask, "left")
        ...
        if i % 100 == 0:
            logger.info(f"  Text encoding: {i}/{len(samples)}")
    ...
    logger.info(f"Loading audio VAE from {ckpt_for_vae}...")

    ac = AudioConditioner(checkpoint_path=ckpt_for_vae, dtype=dtype, device=device)

    logger.info("Encoding audio samples...")
    for idx, sample in enumerate(samples):
        ...
        try:
            ...
            if voice is None:
                logger.warning(f"  Skipping {sample['id']}: no audio")
                continue
            ...
            encode_voice = partial(vae_encode_audio, voice, audio_processor=None)
            # audio_latent = ac(lambda enc: vae_encode_audio(voice, enc, None))
            audio_latent = ac(encode_voice)
        ...
        if idx % 100 == 0:
            logger.info(f"  Audio encoding: {idx}/{len(samples)}")

    del ac
    torch.cuda.empty_cache()

    # ── Step 3: Create dummy video latents ──
    logger.info("Creating dummy video latents...")
    ...
    logger.info(f"  audio_latents: {n_audio} files")
    logger.info(f"  conditions:    {n_cond} files")
    logger.info(f"  latents:       {n_lat} files")
