...
# Local src import.
from dramabox.duration_estimator import estimate_speech_duration
from dramabox.inference_server import TTSServer
from dramabox.model_downloader import get_all_paths
from dramabox.text_chunker import chunk_prompt_for_duration

logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger.info("Fetching DramaBox checkpoints from HuggingFace (cached after first run)...")
PATHS = get_all_paths()
...
def preview_chunks(prompt: str, dur_mult: float, max_chunk_s: float, target_chunk_s: float) -> str:
    ...
    if raw <= max_chunk_s:
        return f"📏 **Estimated duration:** ~{raw:.1f} s · **single-shot** (under {max_chunk_s:.0f} s cap)."
...
def on_generate(
    ...
    def _on_chunk(idx: int, total: int, est_s: float) -> None:
        if total <= 1:
            ...
            progress(idx / total, desc=f"Chunk {idx + 1}/{total} — generating ~{est_s:.1f} s")
    ...
    logger.info(f"Generated in {elapsed:.2f}s (denoise_ref={denoise_ref}) -> {output}")
    return output
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            with gr.Accordion("Inference settings", open=False):
                ...
                dur_slider = gr.Slider(0.8, 2.0, value=1.1, step=0.05, label="Duration × (only used when target duration = 0)")  # noqa: RUF001
                gen_dur_slider = gr.Slider(
                    0.0,
                    600.0,
                    value=0.0,
                    step=1.0,
                    label="Target duration (s) — 0 = auto from prompt; values >45s automatically chunk the prompt (see info under prompt box)",
                )
                ref_dur_slider = gr.Slider(
                    ...
                    value=10.0,
                    step=1.0,
                    label="Reference duration (s) — how many seconds of the uploaded voice reference the model conditions on",
                )
                ...
                denoise_ref_check = gr.Checkbox(
                    ...
                    info=(
                        "Run RE-USE on the uploaded voice reference before "
            ...
            with gr.Accordion("Long-form options (chunking + crossfade)", open=False):
                ...
                max_chunk_slider = gr.Slider(
                    ...
                    info="Hard cap; chunks above this are rare-but-possible tail emissions. The base model degrades past ~45 s.",
                )
                target_chunk_slider = gr.Slider(
                    ...
                    info="Soft cap; new sentences stop being added once a chunk would cross this. 5-10 s headroom under max absorbs estimator under-shoot.",
                )
                crossfade_slider = gr.Slider(
                    ...
                    step=5.0,
                    label="Crossfade (ms)",
                    info="Equal-power overlap between adjacent chunks. 50 ms is inaudible on speech; 0 ms = hard cut.",
                )
    ...
    gr.Examples(
        ...
        examples=[
            ...
            [
                ...
                1.5,
                1.1,
                30.0 if name.startswith("30s") else 0.0,
                10.0,
                42,
                True,
                45.0,
        ...
        inputs=[
            gr.Textbox(visible=False, label="Scene"),
            prompt_box,
            audio_ref,
            cfg_slider,
            stg_slider,
        ...
        outputs=[audio_out],
        fn=lambda _name, prompt, ref, cfg, stg, dur, gen_dur, ref_dur, seed, dn_ref, max_ch, tgt_ch, xfade: on_generate(
            prompt, ref, cfg, stg, dur, gen_dur, ref_dur, seed, dn_ref, max_ch, tgt_ch, xfade
        ),
